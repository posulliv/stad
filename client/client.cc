...
static int read_and_execute(bool interactive)
{
  ...
  for (;;)
  {
    ...
    /* randomize the input query here */
    string output_query(line, strlen(line));
    sql_tokenizer tokenizer(randomization_key);
    tokenizer.randomize_query(output_query);
    free(line);
    line = (char*) calloc(output_query.length(), 1);
    memcpy(line, output_query.c_str(), output_query.length());
    if (! line)
    {
      status.setExitStatus(0);
      break;
